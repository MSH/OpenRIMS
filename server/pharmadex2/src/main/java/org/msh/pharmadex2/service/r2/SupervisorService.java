...
public class SupervisorService {
	...
	public WorkflowDTO workflowConfiguration(WorkflowDTO data) throws ObjectNotFoundException {
		if(data.getDictNodeId()>0) {
			...
			if(url.length()>5){
				...
				throw new ObjectNotFoundException("workflowConfiguration. Configuration url is wrong defined. It is "+url,logger);
			}
		}else {
	...
	public DataConfigDTO dataCollectionVariablesLoad(DataConfigDTO data) throws ObjectNotFoundException {
		if(data.getNodeId()>0) {
			...
			String where="Active=true and nodeID='"+data.getNodeId()+ "' and lang='"+LocaleContextHolder.getLocale().toString().toUpperCase()+"'";
	...
	/**
	 * Get variable classes as variable names in ThingDTO
	 * Reflection!
	 * @return
	 */
	...
	public DataVariableDTO dataCollectionVariableSave(DataVariableDTO data) throws ObjectNotFoundException {
		if(data.getNodeId()>0) {
			...
			if(data.isValid()) {
				...
				node=closureServ.saveToTree(root, node);
				node = literalServ.prefAndDescription(data.getDescription().getValue(), data.getDescription().getValue(), node);
				//save a table record
	...
	public ResourceDTO resourceDefinitionSave(ResourceDTO data) throws ObjectNotFoundException {
		...
		if(data.isValid()) {
			...
			if(data.getNodeId()==0) {
				//prepare resources for all languages, however return a reference for the current one
				...
				resDef=literalServ.prefAndDescription(data.getUrl().getValue(), data.getDescription().getValue(), resDef);
	...
	private Concept resourceDefinitionCreate(ResourceDTO data, Concept root, String localeStr)
		...
		resDef=literalServ.prefAndDescription(data.getDescription().getValue(), data.getDescription().getValue(), resDef);
		return resDef;
	}
	/**
	...
	private MessageDTO loadMessagesTable(MessageDTO data) {
		...
		List<TableRow> rows = jdbcRepo.qtbGroupReport("select * from resource_message", "", mainWhere, table.getHeaders());
		TableQtb.tablePage(rows, table);
